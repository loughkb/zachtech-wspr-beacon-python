...
#version 1.1  Mac serial port detection added
...
def setserial():
    ...
    exists = os.path.exists('/dev/tty.usbserial-1410')
    if exists:
        print("Found tty.usbserial-1410")
        ser = serial.Serial('/dev/ttyusbserial-1410')
        s = ser.read(5)
        ser.close()
        if s == "{MIN}":
            print("Success!  Port is tty.usbserial-1410")
            serialport = "/dev/tty.usbserial-1410"
            return serialport

    exists = os.path.exists('/dev/tty.usbserial-1420')
    if exists:
        print("Found tty.usbserial-1420")
        ser = serial.Serial('/dev/ttyusbserial-1420')
        s = ser.read(5)
        ser.close()
        if s == "{MIN}":
            print("Success!  Port is tty.usbserial-1420")
            serialport = "/dev/tty.usbserial-1420"
            return serialport

    exists = os.path.exists('/dev/tty.usbserial-1430')
    if exists:
        print("Found tty.usbserial-1410")
        ser = serial.Serial('/dev/ttyusbserial-1430')
        s = ser.read(5)
        ser.close()
        if s == "{MIN}":
            print("Success!  Port is tty.usbserial-1430")
...
def checksec(serialport):
    ...
    if not security:
        ...
        print("If you're on Linux, You need to add your user to the dialout group.")
